...
module.exports = function (app) {
  ...
  app.context.checkFile = function (key, deleteOnCheckFailed) {
    ...
    deleteOnCheckFailed =
      "undefined" == typeof deleteOnCheckFailed ? true : false;
...
Validator.prototype.isDate = function (tip) {
  if (
    ...
    (!isString(this.value) || !v.isDate(this.value))
  ) {
...
Validator.prototype.isCurrency = function (tip, options) {
  if (
    this.goOn &&
    (!isString(this.value) || !v.isCurrency(this.value, options))
  ) {
...
Validator.prototype.isMobilePhone = function (tip, locale) {
  if (
    this.goOn &&
    (!isString(this.value) || !v.isMobilePhone(this.value, locale))
  ) {
...
Validator.prototype.encodeBase64 = function () {
  if (this.goOn && !this.hasError() && this.value) {
    this.value = this.params[this.key] = Buffer.from(this.value).toString(
      "base64"
    );
  }
  return this;
  ...
  if (!this.hasError() && this.value) {
    try {
      if (inBuffer) {
        ...
        this.value = this.params[this.key] = new Buffer.from(
          this.value,
          "base64"
        ).toString();
      }
...
Validator.prototype.type = function (t, tip) {
  if (this.value) {
    if (
      ...
      if (!util.types.isDate(this.value))
        this.addError(tip || this.key + " is not date.");
    } else if ("null" == t) {
      if (this.value !== null) this.addError(tip || this.key + " is not null.");
    } else if ("nullorundefined" == t.toLowerCase()) {
      if (!(this.value === null || "undefined" == typeof this.value))
        this.addError(tip || this.key + " is not null or undefined.");
    } else if ("primitive" == t) {
...
FileValidator.prototype.size = function (min, max, tip) {
  if (
    this.goOn &&
    (!this.value || this.value.size < min || this.value.size > max)
  ) {
    this.addError(
      tip ||
        "file " +
