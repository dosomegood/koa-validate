  ...
  it("file check ok", function (done) {
    ...
    app.router.post(
      ...
      convert(function* () {
        ...
        this.checkFile("file")
          .empty()
          .contentTypeMatch(/^application\//);
        yield this.checkFile("file1")
          .empty()
          .move(__dirname + "/temp", function (file, context) {});
        this.checkFile("file").notEmpty();
        yield this.checkFile("file")
          .notEmpty()
        ...
        yield (yield this.checkFile("file")
          .notEmpty()
          .fileNameMatch(/^.*.js$/)
          .size(0, 10 * 1024)
          .suffixIn(["js"])
        ...
        require("fs").unlinkSync(
          __dirname +
            "/" +
            require("path").basename(this.request.files.file.path)
        );
        require("fs").unlinkSync(
          __dirname +
            "/tempdir/" +
